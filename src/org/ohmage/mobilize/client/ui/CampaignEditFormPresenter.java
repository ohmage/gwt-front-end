...
import org.ohmage.mobilize.client.event.UserInfoUpdatedEventHandler;
...
public class CampaignEditFormPresenter {
    ...
    public void initFormForCreate() {
        ...
        view.initializeForm(dataService.authToken(), OhConstants.getCampaignCreateUrl());
        ...
        this.dataService.fetchCampaignDetail(campaignUrn,
                new AsyncCallback<CampaignDetailedInfo>() {
                    ...
                    public void onSuccess(CampaignDetailedInfo result) {
                        ...
                        view.initializeForm(dataService.authToken(), OhConstants.getCampaignUpdateUrl());
                    }
                });
    }
    ...
    private void addEventHandlersToView() {
        ...
        this.view.getSaveButton().addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                ...
                if (validateForm()) {
                    // don't allow double save
                    view.disableSubmitButton();
                    // copies some values into hidden fields
                    view.prepareFormForSubmit(isCreate);
                    view.submitForm();
    ...
    private SubmitCompleteHandler formSubmitCompleteHandler = new SubmitCompleteHandler() {
        ...
        public void onSubmitComplete(SubmitCompleteEvent event) {
            ...
            /*
             * String result =
             * "{\"result\":\"failure\",\"errors\":[{\"text\":\"Campaign already
             * exists.\",\"code\":\"0804 \"}]}"; // sample error response
             */
            /*
             * String result = "<pre style=\"word-wrap: break-word; white-space:
             * pre-wrap;\">{\"result\":\"failure\",\"errors\":[{\"text\":\"The
             * campaign already exists:
             * urn:campaign:ca:lausd:Carson_HS:CS104:Fall:2011:Advertisement\",
             * \"code\":\"0702\"}]}</pre>"; // Safari, with content-type ==
             * application/json
             */
            ...
            /*
             * NOTE: gwt formPanel results can be null if sending to a different
             * domain so you must deploy a compiled version to test this error
             * handling
             */
            if (result != null) {
                try {
                    // in case content-type is not text/html  
                    result = OhDataTranslators.stripPreTags(result);
                    status = JSONParser.parseStrict(result).isObject().get("result").isString().stringValue();
                    if (!"success".equals(status)) {
                        /*
                         * FIXME: refactor OhmageDataService parse/error methods
                         * so they can be reused here (see
                         * parseServerErrorResponse and
                         * getResponseTextOrThrowException)
                         */
